...
######################################## PARAMS #######################################################################
Numberofchordchanges = 100 # total number of chord changes
cpb = 8 # chords span in beats
Fadein = 1000
Fadeout =1000
scale = "C#"
mode = "Aeolian"
...
trail = 1000 # overlap between each chord
HOST = 'localhost'
PORT = 60000 
looporg = "Cymatics - Caribbean Hihat Loop - 103 BPM" # without.wav
loopbpm = 103/2 # numerator needs to match beat loop bmp
#######################################################################################################################
...
def create():
    ...
    for i in range(0,Numberofchordchanges):
        ...
        if(i==0):
            ...
            mixedtest[i*Durationofeachchord:(i+1)*Durationofeachchord].export("export/mixedstream"+str(i)+".wav", format='wav') 
